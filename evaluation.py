...
def convert_to_list(y_aspect, y_sentiment, mask):
    y_aspect_list = []
    y_sentiment_list = []
    for seq_aspect, seq_sentiment, seq_mask in zip(y_aspect, y_sentiment, mask): # get single instance
        l_a = []
        l_s = []
        for label_dist_a, label_dist_s, m in zip(seq_aspect, seq_sentiment, seq_mask): # get single label of the token 
            if m == 0:
                break
            else:
            ...
            if true_seq[num] == begin:
                ...
                if predict[num] == begin:
                    ...
                    for j in range(num+1, len(true_seq)):
                        if true_seq[j] == inside and predict[j] == inside: # continue match
                            flag += 1
                            continue
                        elif true_seq[j] != inside  and predict[j] != inside: # end match! match successfully.
                            break
                        else: # fail to match.
                            match = False
                            break
                    ...
                    if match:
                        correct += 1
                        if not train_op:
                            current_aspect_sentiment = [0, 0, 0, 0]
                            cur_asp_final_senti = predict_sentiment[i][num]
                            if flag > 2:
                                for k in range(num, num+flag+1):
                                    current_aspect_sentiment[predict_sentiment[i][k]] += 1
                                if max(current_aspect_sentiment) > 1:
                                    cur_asp_final_senti = current_aspect_sentiment.index(max(current_aspect_sentiment))
                            ...
                            if true_sentiment[i][num]!=0:
                                ...
                                if true_sentiment[i][num] == predict_sentiment[i][num]:#cur_asp_final_senti
