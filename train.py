...
parser.add_argument("-dual", "--which_dual", dest="which_dual", type=str, metavar='<str>', default='dual', help="which_dual")
parser.add_argument("-b", "--batch-size", dest="batch_size", type=int, metavar='<int>', default=32, help="Batch size (default=32)")
parser.add_argument("--lr", dest="learning_rate", type=float, metavar='<float>', default=0.0001, help="The learing rate. (default=0.0005)")
...
parser.add_argument("--cap_dim", "--capsule_dim", dest="capsule_dim", type=int, metavar='<int>', default=150, help="Embeddings dimension (defa    ult=dim_general_emb + dim_domain_emb = 400)")
...
parser.add_argument("--doc-domain-layers", dest="doc_domain_layers", type=int, metavar='<int>', default=2, help="The number of CNN layers for extracting document domain features")
...
parser.add_argument("--opinion-layers", dest="opinion_layers", type=int, metavar='<int>', default=2, help="The number of CNN lay    ers for extracting aspect features")
...
def shuffle(array_list):
    ...
    for x in array_list:
        ...
        assert len(x) == len_ or len(x) == len_ + 1
...
A, X = pkl.load(open('../data_preprocessed/A/%s_only_A_norm0.5.pkl'%args.domain, 'rb'))
A_train = []
A_test = []

for item in A['train']:
    A_train.append(item[0].toarray())

for item in A['test']:
    A_test.append(item[0].toarray())
A_train = np.array(A_train)
test_A = np.array(A_test)
path = '../'
#code.interact(local=locals())
# split the original training data into train and dev sets
#[train_x, train_A, train_y_aspect, train_y_sentiment, train_y_opinion, train_y_mask], \
#[dev_x, dev_A, dev_y_aspect, dev_y_sentiment, dev_y_opinion, dev_y_mask] = \
...
if args.use_bert:
    berts = []
    with codecs.open(path + 'data_preprocessed/%s/train/sentence.txt.base_bert-%s_%s'%(args.domain, args.bert_layers, args.bert_mode), 'r') as f:
        for line in f:
            sentence_length = len(line.strip().split('|||'))
            sentence_bert = [[float(value) for value in values.split()] for values in line.strip().split('|||')] + [[0.0] * 768] * (overall_maxlen +1 - sentence_length)
            berts.append(sentence_bert)
            assert len(berts[0][0]) == 768
            #code.interact(local=locals())
    test_berts = []
    with codecs.open(path + 'data_preprocessed/%s/test/sentence.txt.base_bert-%s_%s'%(args.domain, args.bert_layers, args.bert_mode), 'r') as f1:
        for line in f1:
            sentence_length = len(line.strip().split('|||'))
            sentence_bert = [[float(value) for value in values.split()] for values in line.strip().split('|||')] + [[0.0] * 768] * (overall_maxlen +1 - sentence_length)
    ...
    # split the original training data into train and dev sets
    [train_x, train_A, train_berts, train_y_aspect, train_y_sentiment, train_y_opinion, train_y_mask], \
    [dev_x, dev_A, dev_berts, dev_y_aspect, dev_y_sentiment, dev_y_opinion, dev_y_mask] = \
...
save_model = False
if args.use_bert:
    gen_aspect_bert = batch_generator([train_x, train_A, train_berts, train_y_aspect, train_y_sentiment, train_y_opinion, train_y_mask], batch_size=args.batch_size)
else:
    gen_aspect = batch_generator([train_x, train_A, train_y_aspect, train_y_sentiment, train_y_opinion, train_y_mask], batch_size=args.batch_size)
...
for ii in xrange(args.epochs):
    ...
    for b in xrange(batches_per_epoch_aspect):
        if args.use_bert:
            batch_x, batch_A, batch_bert, batch_y_ae, batch_y_as, batch_y_op, batch_mask = gen_aspect_bert.next()
            #code.interact(local=locals())
            loss_, loss_aspect_, loss_opinion_, loss_sentiment_ = aspect_model.train_on_batch([batch_x, batch_A, batch_y_op, gold_prob, batch_bert], [batch_y_ae, batch_y_op, batch_y_as])
        else:
            batch_x, batch_A, batch_y_ae, batch_y_as, batch_y_op, batch_mask = gen_aspect.next()
            loss_, loss_aspect_, loss_opinion_, loss_sentiment_ = aspect_model.train_on_batch([batch_x, batch_A, batch_y_op, gold_prob], [batch_y_ae, batch_y_op, batch_y_as])
    ...
    if args.use_bert:
        y_pred_aspect, y_pred_opinion, y_pred_sentiment = aspect_model.predict([dev_x, dev_A, dev_y_opinion, np.zeros((len(dev_x), overall_maxlen)), dev_berts])
    else:
        y_pred_aspect, y_pred_opinion, y_pred_sentiment = aspect_model.predict([dev_x, dev_A, dev_y_opinion, np.zeros((len(dev_x), overall_maxlen))])

    f_opinion,_, _, _, _ \
         = get_metric(dev_y_opinion, y_pred_opinion, dev_y_sentiment, y_pred_sentiment, dev_y_mask, args.train_op)

    f_aspect, _, acc_s, f_s, f_absa \
    ...
    if f_absa > best_dev_metric and ii > 40:
        best_dev_metric = f_absa
        save_model = True
    else:
        save_model = False
    if args.use_bert:
        y_pred_aspect, y_pred_opinion, y_pred_sentiment = aspect_model.predict([test_x, test_A, test_y_opinion, np.zeros((len(test_x), overall_maxlen)), test_berts])
    else:
        y_pred_aspect, y_pred_opinion, y_pred_sentiment = aspect_model.predict([test_x, test_A, test_y_opinion, np.zeros((len(test_x), overall_maxlen))])

    f_opinion,_, _, _, _ \
        = get_metric(test_y_opinion, y_pred_opinion, dev_y_sentiment, y_pred_sentiment, dev_y_mask, args.train_op)
    ...
    logger.info('Test results -- [Aspect f1]: %.4f, [Opinion f1]: %.4f, [Sentiment acc]: %.4f, [Sentiment f1]: %.4f, [Overall f1]: %.4f' 
                        %(f_aspect, f_opinion, acc_s, f_s, f_absa))
    print('Test results:', f_aspect, f_opinion, acc_s, f_s, f_absa)
    if f_absa > best_test_metric and save_model == True:
        best_test_metric = f_absa
    if save_model == True:
        print '-------------- Save model --------------'
        print 'current best test f_absa is ', best_test_metric
        logger.info('current best test f_absa is %.6f'%best_test_metric)
